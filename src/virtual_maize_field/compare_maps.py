...
def compute_score(gt, pred):
    ...
    return (
        best_score,
        np.array(best_gt_matches),
        np.array(best_pred_matches),
        np.array(best_dist),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Path of the prediction csv file, generated by the competitor performing"
            " the mapping task"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out_dpi", type=int, default=1000, help="DPI of the output map"
    )
